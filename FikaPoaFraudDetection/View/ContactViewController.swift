...
class ContactViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let toOtpViewController = segue.destination as? OTPViewController{
            
            toOtpViewController.fname = fname
            toOtpViewController.middlenames = middlenames
            toOtpViewController.lastname = lastname
            toOtpViewController .occupation = occupation
            toOtpViewController.dateofbith = dateofbith
            toOtpViewController.genders = genders
            toOtpViewController.permanent_address = permanet_address.text
            toOtpViewController .current_address = current_address.text
            toOtpViewController.email_address = emailaddress.text
            toOtpViewController.pincode = pincode.text
            toOtpViewController.country = country.text
            toOtpViewController.state = state.text
            toOtpViewController.city = city.text
            toOtpViewController.areaofResident = areaofResident.text
            toOtpViewController.phone_no = phone_no.text
            otpResponse = "3000"
            print("Setting otp property to: \(self.otpResponse))")
            toOtpViewController.getOtp = self.otpResponse
           
        }
    }
    @IBAction func submitbtTapped(_ sender: Any) {
        let nextTo: Void = performSegue(withIdentifier: "otpveryfy", sender: self)
        do{
        ...
        let datas = createParameters()
            guard let Url = URL(string: "http://localhost:3001/otp") else{return}
                    ...
                    if let error = error{
                        ...
                        if let data = data, let response = response as? HTTPURLResponse, response.statusCode == 200 {
                            do {
                                ...
                                otpResponse = json?["otp"] as? String
                                print("OTP received: \(otpResponse ?? "N/A")")
                                // Store the OTP for user verification
                                
                                
                                print("this is the response\( self.otpResponse ?? "")")
                                nextTo
                            } catch {
            ...
            let alart = UIAlertController(title: "", message: "Check your internet connection", preferredStyle: .alert)
    ...
    func  registeringData(permanentAddress:String,CurrentAddress:String,mobile:String,emailAddress:String,Pin:String,City:String,State:String,Country:String){
        ...
        if((reachability.connection) != .unavailable){
            ...
            let domains:String = "http://localhost:8080/registration.php"
            ...
            do{
                ...
                //print(AF.request(domains, method: .post, parameters: sendData).responseData{})
                            ...
                            if let responsMessage = json as? [String:Any],
                               let responseString = responsMessage["message"] as? String{
                                ...
                                let close = UIAlertAction(title: "Close", style: .cancel, handler: { action in
                                    ...
                                    self.performSegue(withIdentifier: "otpveryfy", sender: self)
                                })
    ...
    func createParameters() -> [String: Any] {
        ...
        parameters["phone_no"] = countryCode + phone_no.text!.trimmingCharacters(in: .whitespacesAndNewlines).replacingOccurrences(of: "^0+", with: "", options: .regularExpression)
        generateOTP["phone_no"] = countryCode + phone_no.text!.trimmingCharacters(in: .whitespacesAndNewlines).replacingOccurrences(of: "^0+", with: "", options: .regularExpression)
        return generateOTP
    }
